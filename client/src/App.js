...
function App() {
  ...
  const addItem = async (e) => {
   ...
   console.log('process.env.REACT_APP_BACKEND_URL',process.env.REACT_APP_BACKEND_URL)
    try{
      const res = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/api/item`, {item: e.target.item.value, assignee:e.target.assignee.value})
  ...
  useEffect(()=>{
    const getItemsList = async () => {
      try{
        const res = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/items`)
  ...
  const deleteItem = async (id) => {
    try{
      const res = await axios.delete(`${process.env.REACT_APP_BACKEND_URL}/api/item/${id}`)
